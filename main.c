...
int main (int argc, char** argv) {
	...
	if (joystick_fd < 0) perror("during joystick acquisition");

	/* Create socket */
	int sockfd = socket(AF_INET, SOCK_STREAM, 0);
	if (sockfd < 0) perror("on socket creation");

	/* Acquire host IP */
	struct hostent *server = gethostbyname(argv[1]);
	if (!server) perror("on getting server ip");
	...
	if (connect(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0) 
		perror ("when connecting to server");
	...
	while (read(joystick_fd, &event, sizeof(struct js_event)) == sizeof(struct js_event)) {
		...
		if (write(sockfd, message_buf, buf_len) < buf_len)
			perror("on socket write");
	}
	perror("on joystick read");
